"""Routing evals for ticket tool vs RAG paths in the support agent graph."""
...
if str(INCIDENT_API_ROOT) not in sys.path:
    sys.path.insert(0, str(INCIDENT_API_ROOT))

from agents.support_agent.graph import run_agent
from agents.support_agent.trace import clear_traces, get_trace
from agents.tools.incident_lookup import TicketLookupInput, lookup_incident


class _TestClientTransport(httpx.BaseTransport):
    """Bridge FastAPI TestClient to httpx for lookup_incident integration tests."""
...
@pytest.fixture
def incident_lookup_via_app(tmp_path, monkeypatch):
    ...
    def _lookup(tool_input: TicketLookupInput):
    ...
    return _lookup, incident_id


def test_eval_routes_ticket_question_to_tool(incident_lookup_via_app) -> None:
    """Tool-routed question uses ticket lookup against the real incident API."""
    lookup_fn, incident_id = incident_lookup_via_app
    clear_traces()
...
def test_eval_routes_policy_question_to_rag() -> None:
    ...
    assert "ticket_lookup_node" not in node_names
    assert result["sources_used"] == ["rag"]


def test_eval_ticket_fallback_on_not_found(incident_lookup_via_app) -> None:
    """Missing ticket IDs return an honest fallback without inventing status."""
    lookup_fn, _incident_id = incident_lookup_via_app
    clear_traces()
    ...
    trace = get_trace(result["trace_id"])
    ticket_step = next(
        entry for entry in trace if entry["node"] == "ticket_lookup_node"
    )
    assert ticket_step["output_summary"]["ok"] is False
    assert ticket_step["output_summary"]["error_code"] == "not_found"


def test_eval_ticket_fallback_on_timeout(incident_lookup_via_app) -> None:
    """Timeouts surface a user-safe message instead of fabricated ticket data."""
    _lookup_fn, incident_id = incident_lookup_via_app
    clear_traces()
