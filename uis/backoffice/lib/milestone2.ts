...
export const buildExecutiveSnapshot = (data: CountryKpiInput[]): ExecutiveSnapshot => {
  const globalShipmentVolume = data.reduce((sum, row) => sum + row.shipments, 0);

  const totalOnTimeDeliveries = data.reduce((sum, row) => sum + row.onTimeDeliveries, 0);

  const totalReturns = data.reduce((sum, row) => sum + row.returnedOrders, 0);

  const totalOperatingCostUsd = data.reduce((sum, row) => sum + row.operatingCostUsd, 0);

  const averageCsat =
    data.length === 0
      ? 0
      : round(data.reduce((sum, row) => sum + row.customerSatisfaction, 0) / data.length);

  const globalOnTimeRate =
    globalShipmentVolume === 0 ? 0 : round((totalOnTimeDeliveries / globalShipmentVolume) * 100);

  const globalReturnRate =
    globalShipmentVolume === 0 ? 0 : round((totalReturns / globalShipmentVolume) * 100);
  ...
  return {
    ...
    countryBreakdown: data.map((row) => ({
      ...
      onTimeRate: row.shipments === 0 ? 0 : round((row.onTimeDeliveries / row.shipments) * 100),
      returnRate: row.shipments === 0 ? 0 : round((row.returnedOrders / row.shipments) * 100),
